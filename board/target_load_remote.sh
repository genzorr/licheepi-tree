...
# Update kernel.
rsync -avzh -e "sshpass -p $PASSWD ssh -p $PORT" output/images/zImage	$USER@f.opteh.ru:/boot/
rsync -avzh -e "sshpass -p $PASSWD ssh -p $PORT" output/images/boot.scr	$USER@f.opteh.ru:/boot/
